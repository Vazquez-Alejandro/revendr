...
app.get('/crm/pipeline', async (req, res) => {
  try {
    ...
    let query = db.collection('leads').where('user_id', '==', userId)
    ...
    res.json({ success: true, data: { pipeline: stageOrder.map(s => ({ stage: s, label: labels[s], count: pipeline[s].length, leads: pipeline[s].slice(0, 50) })), total: snapshot.size } })
...
app.post('/crm/leads/:leadId/stage', async (req, res) => {
  try {
    ...
    if (!leadDoc.exists || leadDoc.data().user_id !== req.user.uid) return res.status(404).json({ success: false, error: { message: 'Lead not found' } })
    await db.collection('leads').doc(req.params.leadId).update({ crm_stage: stage, fecha_actualizacion: new Date() })
    await db.collection('crm_events').add({ lead_id: req.params.leadId, user_id: req.user.uid, event_type: 'stage_change', new_stage: stage, description: `Cambio a ${({ nuevo: 'Nuevo', contactado: 'Contactado', interesado: 'Interesado', negociacion: 'Negociación', cerrado: 'Cerrado', perdido: 'Perdido' }[stage]) || stage}`, timestamp: new Date() })
...
app.post('/crm/leads/:leadId/activity', async (req, res) => {
  try {
    ...
    const leadDoc = await db.collection('leads').doc(req.params.leadId).get()
    if (!leadDoc.exists || leadDoc.data().user_id !== req.user.uid) return res.status(404).json({ success: false, error: { message: 'Lead not found' } })
    await db.collection('crm_events').add({ lead_id: req.params.leadId, user_id: req.user.uid, event_type: type || 'note', description: description || '', value: value || null, timestamp: new Date() })
...
app.get('/crm/leads/:leadId/timeline', async (req, res) => {
  try {
    const snapshot = await db.collection('crm_events').where('lead_id', '==', req.params.leadId).where('user_id', '==', req.user.uid).orderBy('timestamp', 'desc').limit(50).get()
...
app.put('/crm/events/:eventId', async (req, res) => {
  try {
    ...
    const eventDoc = await db.collection('crm_events').doc(req.params.eventId).get()
    if (!eventDoc.exists || eventDoc.data().user_id !== req.user.uid) return res.status(404).json({ success: false, error: { message: 'Event not found' } })
